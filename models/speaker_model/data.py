...
class data:
    """
    Data Form:
        Assume the data is of this form:
        SpeakerId Text|AddresseeId Text
    """
    ...
    def read_batch(self, file, num, mode='train_or_test'):
        ...
        for i in range(self.params.batch_size):
            ...
            source = self.encode(s[1:])     # Assume SpeakerId is one digit number
        ...
        # Finalize
        try:
            max_l_s, max_l_t = max(l_s_set), max(l_t_set)
        except ValueError:
        ...
        return END, \
            tensor(sources).long(), tensor(targets).long(), \
            tensor(speaker_label).long(), tensor(addressee_label).long(), \
            tensor(length_s).long(), token_num, origin
