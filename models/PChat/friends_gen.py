...
model.load_state_dict(torch.load('/home/junkang13/models/minigpt/model9_150.pth'))
...
with open('val_fake_dataset.json', 'r') as file:
    data = json.load(file)
...
for entry in data[:50]:
    ...
    user2_persona = entry.get("persona", None)
    context = entry.get("conversation", None)
    all_generated = []
    if context:
        for i in range(len(context)):
            input_text = ""
            ...
            padding_length = 80 - input.shape[1]
            padded_input_ids = torch.cat([torch.full((1, padding_length), tokenizer.pad_token_id), input], dim=1).squeeze(0).to(torch.long)

            model.eval()
            generated = tokenizer.decode(model.generate(padded_input_ids, user2_persona, max_new_tokens=30).squeeze().tolist())
            # try:
            bot_response = clean_generated_text(generated, input_text)
            # except:
            #     print(input_text)

            current_turn = list(context[i].items())
            if len(current_turn) > 1:
                user_speaker = current_turn[1][0]
                user_utterance = current_turn[1][1]
            else:
                user_speaker = "user_response"
                user_utterance = "None"

            bot_speaker = current_turn[0][0]

            all_generated.append({bot_speaker: bot_response, user_speaker: user_utterance})

            # all_generated.append({"user": context[i]['user'], "bot": bot_response})


    result_entry = {
        "persona": user2_persona,
        "gt_conversations": context,
...
with open('friends_results.json', 'w', encoding='utf-8') as file:
    json.dump(results, file, ensure_ascii=False, indent=4)

print("所有记录处理完毕并已保存到 friends_results.json")