...
class MiniGPT(nn.Module):
    ...
    def generate(self, context, p, max_new_tokens=100):
        context = context.clone().detach().unsqueeze(0).to(self.device)
        output_token = []
        for i in range(max_new_tokens):
            in_token = context[:, 64:] if context.size(1) >= 120 else context
    ...
    def __iter__(self):
        for user2_persona, context in self.data:
            ...
            for i in range(len(context)):
                if i == 0:
                    input_text = f"User: {context[i].get('user','None')}"
                elif i == 1:
                    input_text = f"User: {context[i-1].get('user','None')} Bot: {context[i-1].get('bot','None')} User: {context[i].get('user','None')}"
                else:
                    input_text = f"User: {context[i-2].get('user','None')} Bot: {context[i-2].get('bot','None')} User: {context[i-1].get('user','None')} Bot: {context[i-1].get('bot','None')} User: {context[i].get('user','None')}"
                #print(context[i])
                target_text = input_text + f" Bot: {context[i].get('bot','None')}"
            ...
            #     target_text = input_text+target_text
                ...
                padding_length = 150 - input_ids.shape[1]
...
def clean_generated_text(generated_text, context):
    if context in generated_text:
        cleaned_text = generated_text.split(context, 1)[-1].strip()
    else:
        cleaned_text = generated_text.strip()
    cleaned_text = cleaned_text.replace(tokenizer.eos_token, "").strip()
    return cleaned_text

model.load_state_dict(torch.load("model9_250.pth"))
model.to('cuda' if torch.cuda.is_available() else 'cpu')
with open('test_spc_dataset.json', 'r') as file:
    data = json.load(file)
...
for entry in data:
    user1_persona = entry.get("user1_persona", None)
    user2_persona = entry.get("user2_persona", None)
    conversations = entry.get("conversations", None)
    all_generated = []
    if conversations:
        for i in range(len(conversations)):
            if i == 0:
                context = f"User: {conversations[i].get('user','None')}"
            elif i == 1:
                context = f"User: {conversations[i-1].get('user','None')} Bot: {conversations[i-1].get('bot','None')} User: {conversations[i].get('user','None')}"
            else:
                context = f"User: {conversations[i-2].get('user','None')} Bot: {conversations[i-2].get('bot','None')} User: {conversations[i-1].get('user','None')} Bot: {conversations[i-1].get('bot','None')} User: {conversations[i].get('user','None')}"
            ...
            padding_length = 150 - input.shape[1]
            padded_input_ids = torch.cat([torch.full((1, padding_length), tokenizer.pad_token_id), input], dim=1).squeeze(0)

            model.eval()
            generated = tokenizer.decode(model.generate(padded_input_ids, user2_persona, max_new_tokens=60).squeeze().tolist())
            bot_response = clean_generated_text(generated, context)
            all_generated.append({"user": conversations[i]['user'], "bot": bot_response})

    result_entry = {
        "user1_persona": user1_persona,
        "user2_persona": user2_persona,
        "gt_conversations": conversations,
        "generated_conversations": all_generated
    }
    results.append(result_entry)
    print(count)
    count+=1

with open('generated_results.json', 'w', encoding='utf-8') as file:
    json.dump(results, file, ensure_ascii=False, indent=4)

print("所有记录处理完毕并已保存到 generated_results.json")