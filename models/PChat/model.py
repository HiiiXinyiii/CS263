...
@dataclass
class MiniGPTConfig:
    path_to_data: Path = Path("train_spc_dataset.json")
    batch_size: int = 32
    num_layers: int = 8
    vocab_size: int = 50257
    embed_dim: int = (
        192  # Dimensionality of the the token embeddings (throught the transformer)
    )
    ...
    def __init__(self, input_dim, num_heads, dropout=0.1) -> None:
        ...
        for i in range(num_heads):
            setattr(self, f'head_{i}', SingleHeadAttention(
                ...
                dropout=dropout,
                max_len=256  # Assuming max_len is 512, adjust if necessary
            ))
...
class PersonaEmbedding(nn.Module):
    ...
    def forward(self, persona_text):
        ...
        try:
            inputs = self.tokenizer(persona_text, return_tensors="pt", padding='max_length', truncation=True, max_length=64)
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
        except:
            print('persona_text:', persona_text)
            inputs = self.tokenizer('No such persona, and I do not know what happened.', return_tensors="pt", padding='max_length', truncation=True, max_length=64)
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
...
class MiniGPT(nn.Module):
    ...
    def generate(self, context, p, max_new_tokens=100):
        ...
        for i in range(max_new_tokens):
            in_token = context[:, -256:] if context.size(1) >= 10 else context
...
class TinyStoriesDataset(IterableDataset):
    ...
    def load_data(self):
        ...
        dialogue_batches = []
        for entry in data:
            user2_persona = entry.get("user2_persona", "No user2 persona")
            context = entry.get("conversations", [])
            # next_item = entry.get("next_conversation", {"user": "No next user", "bot": "No next bot"})  # 假设next_item是一个字典
    ...
    def __iter__(self):
        for user2_persona, context in self.data:
            for i in range(len(context) - (self.context_length + 1)):
                persona_text = user2_persona
                current_context = context[i:i + self.context_length]
                additional_user_speech = context[i + self.context_length].get('user', '|<eos>|')
                ...
                yield input_ids, target_ids, persona_text


    def __len__(self):
        return len(self.data) - self.context_length

model = MiniGPT(MiniGPTConfig).to('cuda')
model.load_state_dict(torch.load('models/minigpt/model9_400.pth'))
...
eval_dataset = TinyStoriesDataset(
    "val_spc_dataset.json", tokenizer = tokenizer, context_length=2
)
...
tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
model.load_state_dict(torch.load("models/minigpt/model8_800.pth"))
# model.load_state_dict(torch.load("../../../model_300.pth", map_location=torch.device('cpu')))
model.to('cuda' if torch.cuda.is_available() else 'cpu')

gen_sent = "User: It's good to take small steps towards being more open. I used to be shy too, but I've been trying to step out of my comfort zone. Bot: That's wonderful progress! I believe being open to new experiences can lead to amazing opportunities. User: Thank you for the encouragement. What do you have planned for the weekend? Bot: I'm planning to start the day with a cold shower and then spend some time gardening. It's a relaxing way for me to unwind. User: Gardening sounds like a peaceful activity. Each plant tells a unique story as it grows."
gen_tokens = torch.tensor(tokenizer.encode(gen_sent))
p = "I have a dog and a cat, both of whom are very well-behaved. My mother is a librarian. I want to learn more about how to overcome my fears and live a more fulfilling life. I speak fluent German. I love the fact that my job allows me to travel and meet new people. MBTI: INFJ"
# print("Generating text starting with:", gen_tokens.shape)
gen_tokens = gen_tokens.to("cuda")
model.eval()
print(
    tokenizer.decode(
        model.generate(gen_tokens, p, max_new_tokens=160).squeeze().tolist()
    )
)
